...
1. **Label provenance** — how much does training-label quality (NWI vs. field-verified)
   affect multiclass wetland mapping skill, and is any gap driven by *quantity* (fewer
   wetland pixels) or *correctness* (the wrong pixels)?
2. **Below-canopy feature contribution** — which LiDAR-derived features and which
   ...
   above-canopy leaf-on spectral data cannot.
