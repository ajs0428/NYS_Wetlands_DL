...
lidar_index_all_sf <- st_read("Data/Lidar/NYS_Lidar_All_Indexes.gpkg", quiet = TRUE) |> 
    dplyr::mutate(COLLECTION = case_when(is.na(COLLECTION) ~ COLLECTION_NAME,
                                         .default = COLLECTION)) |> 
    dplyr::select(-COLLECTION_NAME) 
lidar_index_all_sf <- lidar_index_all_sf[!st_is_empty(lidar_index_all_sf), ]

lidar_huc <- function(huc_num){
    huc <- cluster_hucs[cluster_hucs$huc12 == huc_num, ]
    
    index_intersect <- st_intersects(lidar_index_all_sf, huc, sparse = F)
    
    index_in_huc <- lidar_index_all_sf[rowSums(index_intersect) > 0, ]
