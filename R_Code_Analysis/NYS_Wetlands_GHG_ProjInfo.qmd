...
ny_hucs_100 <- ny_hucs_clust[ny_hucs_clust$cluster == 93,]
...
#### Stopped here after getting the function to return a wrapped sprc that can be merged later

```{r}
huc_extract_merge <- function(cluster){
    f_list <- list()
    for(i in seq_along(cluster$huc12[1:3])){
        dem_to_extr <- cluster_extract(cluster = cluster[i, ], dem_indexes = st_list_trs)
        f_list[[i]] <- dem_file_list[basename(dem_file_list) %in% dem_to_extr]
    }
    
    r_list <- foreach(i = seq_along(cluster$huc12[1:3]), .packages = c("sf", "terra", "tidyterra")) %dopar% {
        r_col <- lapply(f_list[[i]], terra::rast) 
        v_clust <- terra::vect(cluster[i,]) |> 
            terra::project(crs(unwrap(r_col[[i]])))
        s_col <- wrap(sprc(r_col))
        # r_dem <- terra::merge(s_col) |>
        #     terra::mask(mask = v_clust, updatevalue=NA, touches = FALSE,
        #                 filename = paste0(args[5],"cluster_",args[3], cluster$huc12[i], "_dem.tif"),
        #                 overwrite = TRUE)
        # r_dem_wrap <- terra::wrap(r_dem)
        # return(r_dem_wrap)
    }
    return(r_list)
}
...
for(i in seq_along(1:3)){
    dem_to_extr <- cluster_extract(cluster = test_clust[i, ], dem_list = st_list_trs)
    f_list[[i]] <- dem_file_list[basename(dem_file_list) %in% dem_to_extr]
}
    
r_list <- list()
for(i in seq_along(1:1)){
    r_col <- lapply(f_list[[i]], terra::rast) #terra::sprc(f_list[[i]])
    v_clust <- terra::vect(test_clust[i,]) |> terra::project(crs(r_col[[i]]))
    s_col <- sprc(r_col)
    r_list[[i]] <- terra::merge(s_col) |> terra::crop(y = v_clust, mask = TRUE)
}
...
huc_dems_unwrap<- lapply(unlist(huc_dems), terra::unwrap)

cluster_dem <- terra::merge(huc_dems_unwrap[[3]])
```
