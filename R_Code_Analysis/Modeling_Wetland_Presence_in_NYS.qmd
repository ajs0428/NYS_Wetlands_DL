...
pts_extracted_df <- as_tibble(pts_extracted_filtered) |> 
    # unite(dplyr::starts_with("X"), sep = "", col = "CHM") |> 
    # mutate(CHM = str_remove_all(CHM, "[A-Z]"),
    #        CHM = as.numeric(CHM)) |> 
    ...
    dplyr::select(MOD_CLASS, COARSE_CLASS, ID, everything(), -geom, -flowacc.1, -twi.1, -contains("3x3"), -contains("10m")) |> 
    drop_na()
...
Wetland_Multi_Recipe <- 
    recipe(MOD_CLASS ~ ., data = train_data |> select(-ID, -COARSE_CLASS, -huc, -ID)) 
Wetland_Coarse_Recipe <- 
    recipe(COARSE_CLASS ~ ., data = train_data |> select(-ID, -MOD_CLASS, -huc, -ID)) 


RFM <- 
    rand_forest(
        mtry = ncol(train_data|> select(-ID))-3,
        trees = 100,
        min_n = 3
    ) |> 
    set_engine(
