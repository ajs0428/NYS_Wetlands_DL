...
cluster_df <- as_tibble(cluster_pts) |> dplyr::select(-COARSE_CLASS) |> 
    ...
    dplyr::select(MOD_CLASS, ID, everything())
```
...
pc <- prcomp(cluster_df_pca|> select(-MOD_CLASS, -ID),
             center = TRUE,
            scale. = TRUE)
...
Wetland_Class_Recipe <- 
    recipe(MOD_CLASS ~ ., data = train_data |> select(-ID)) 


RFM <- 
    rand_forest(
        mtry = ncol(train_data|> select(-ID))-1,
        trees = 500,
        min_n = 5
    ) |> 
    set_engine(
