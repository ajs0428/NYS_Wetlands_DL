...
### Key scripts
**Python (`Python_Code_Analysis/DL_Pipeline_v2/`):** `dl_experiment_config.py` (config matrix, source of truth) · `dl_make_config_stats.py --all` (8 per-config stats from the master) · `dl_preflight_check.py` (HARD GATE before GPU time; `--seeds` defaults to R=5 in v3) · `dl_08_aggregate_factorial.py` (Phase-3 aggregation, **CPU/pandas**) · `dl_08b_aggregate_patchcurve.py` (follow-on aggregation: patch-curve + `--arch-compare`, **CPU**) · `dl_09_shap_factorial.py` (SHAP, **GPU/in-container**) · `dl_huc_stack.py` (build HUC inference stack) · `dl_10_factorial_viz.ipynb` (lightweight figures §1-§7 from CSV/JSON, **CPU**, git-syncable) · `dl_10b_huc_inference_viz.ipynb` (data-heavy §8 from prediction GeoTIFFs, **CPU**).
